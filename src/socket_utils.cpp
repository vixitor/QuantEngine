...
auto getIfaceIP(const std::string& iface) -> std::string {
  ...
  if (getifaddrs(&ifaddr) != -1) {
    for (ifaddrs* ifa = ifaddr; ifa; ifa = ifa->ifa_next) {
      if (ifa->ifa_addr and ifa->ifa_addr->sa_family == AF_INET and iface == ifa->ifa_name) {
...
auto wouldBlock() -> bool { return (errno == EWOULDBLOCK || errno == EINPROGRESS); }
...
auto createSocket(Logger& logger, const std::string& t_ip, const std::string& iface, int port,
  ...
  logger.log(
      "%:% %() % ip:% iface:% port:% is_udp:% is_blocking:% is_listening:% ttl:% SO_time:%\n",
      __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str), ip, iface, port, is_udp,
  ...
  for (addrinfo* rp = result; rp; rp = rp->ai_next) {
    ...
    if (is_blocking == false) {
      ...
      if (is_udp == false and setNoDelay(fd) == false) {
        logger.log("setNoDelay() failed. errno:%\n", strerror(errno));
        return -1;
      }
    }
    if (is_listening == false and connect(fd, rp->ai_addr, rp->ai_addrlen) == -1 &&
        wouldBlock() == false) {
