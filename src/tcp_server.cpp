...
auto TCPServer::defaultRecvCallback(TCPSocket* socket, Nanos rx_time) noexcept -> void {
  logger_.log("%:% %() % TCPServer::defaultRecvCallback() socket:% len:% rx:%\n", __FILE__,
              __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_), socket->fd(),
...
auto TCPServer::epoll_add(TCPSocket* socket) -> bool {
  ...
  return (epoll_ctl(efd_, EPOLL_CTL_ADD, socket->fd(), &ev) != -1);
}

auto TCPServer::epoll_del(TCPSocket* socket) -> bool {
  return (epoll_ctl(efd_, EPOLL_CTL_DEL, socket->fd(), nullptr) != -1);
}
...
auto TCPServer::poll() noexcept -> void {
    ...
    if (event.events & EPOLLIN) {
      if (socket == &listener_socket_) {
        logger_.log("%:% %() % EPOLLIN listener_socket:%\n", __FILE__, __LINE__, __FUNCTION__,
                    getCurrentTimeStr(&time_str_), socket->fd());
  ...
  while (have_new_connection) {
    ...
    int fd = accept(listener_socket_.fd(), reinterpret_cast<sockaddr*>(&addr), &addr_len);
    if (fd == -1)
      break;
    ...
    logger_.log("%:% %() % accepted socket:%\n", __FILE__, __LINE__, __FUNCTION__,
                getCurrentTimeStr(&time_str_), fd);
    TCPSocket* socket = new TCPSocket(logger_);
    socket->setFd(fd);
    socket->setRecvCallback(recv_callback_);
