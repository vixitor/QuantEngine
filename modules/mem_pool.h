...
public: 
    explicit MemPool(std::size_t num_elems) : store_(num_elems, {T(), true}){
        ASSERT(reinterpret_cast<Object*>(&(store_[0].object_)) == &(store_[0]), "T should be the first element of Object Block" );
    }
    MemPool() = delete;
    ...
    T *allocate(Args... args) noexcept { 
        ...
        ASSERT(obj_block->is_free_ == true, "Expect free Object Block at index: " + std::to_string(next_free_index));
    ...
    auto deallocate(T *elem) { 
        ...
        ASSERT(obj_index >= 0 and static_cast<std::size_t>(obj_index) < store_.size(), "Deallocate element does not belong to the MemPool");
        ASSERT(store_[obj_index].is_free_ == false, "Expect in-use Object Block at index " + std::to_string(obj_index));
